...
const tryReturnValidTransformState = (bubbles,action) =>{
	...
	const AssociateForcingAlgorithm = (bubblekey) =>{
		console.log(bubbles[bubblekey].ParentAssociatedBubble)
	...
	const TransformBubble = (bubblekey,part,amount,keynottochange) =>{
		...
		const originalWidth = bubbles[bubblekey].enddate-bubbles[bubblekey].startdate
		switch (part) {
			case 'start':
	...
	bubbles[key].y = changes.y || bubbles[key].y
	var part = ''
	var amountToShift = 0
	...
	//if(!Object.keys(bubbles).every(bubblekey => {var bubble = bubbles[bubblekey];return checkForNoSameLine(bubble,bubbles)})){console.log('same line!');return false}

	return bubbles
}

const checkForNoSameLine = (bubble,bubbles) => {
