    ...
    return (
      <div className={style.dropdown || "dropdown"} style={style} onBlur={this.props.onBlur}>
        ...
        <ul className={style.dropdownMenu ||"dropdown-menu"}>
          {Object.keys(this.props.options).map((v,index) => <li className={style.dropdownItem ||"dropdown-item"} autoFocus key={index} onMouseDown={() => this.props.submit(v)}>{this.props.options[v]}</li>)}
        </ul>
      </div>);
  }
}
