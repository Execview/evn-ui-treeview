...
const tryReturnValidTransformState = (bubbles,key,changes) =>{
	...
	if(!Object.keys(bubbles).every(bubblekey => {
		var bubble = bubbles[bubblekey]
		const validStartAndEnd = bubble.startdate<bubble.enddate || !bubble.startdate || !bubble.enddate
		if(!validStartAndEnd){console.log(`${bubblekey} has some invalid data`)}
